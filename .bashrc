...
# Force color prompt if supported
...
get_rainbow_hostname() {
	local text=$(hostname)                       # Get the actual hostname
	local colors=(196 202 208 226 118 39 99 201) # Red, Orange, Yellow, Green, Blue, Indigo, Violet
	local rainbow=""
	local index=0
	for ((i = 0; i < ${#text}; i++)); do
...
if [ "$color_prompt" = yes ]; then
	...
	prompt_command() {
		...
		local rainbow_hostname=$(get_rainbow_hostname)
...
export PATH="$HOME/bin/projetos/pesquisa:$PATH"
...
[ -f "$HOME/.fzf.bash" ] && source "$HOME/.fzf.bash" || exit 1 
